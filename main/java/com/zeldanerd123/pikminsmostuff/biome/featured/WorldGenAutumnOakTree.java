package com.zeldanerd123.pikminsmostuff.biome.featured;
...
public class WorldGenAutumnOakTree extends WorldGenAbstractTree
{
    ...
    public WorldGenAutumnOakTree(Block wood, Block leaves, int metaWood, int metaLeaves)
    {
        this(wood, leaves, metaWood, metaLeaves, false, 4, 3, false);
    }

    public WorldGenAutumnOakTree(Block wood, Block leaves, int metaWood, int metaLeaves, boolean doBlockNotify, int minTreeHeight, int randomTreeHeight, boolean vinesGrow)
    {
        super(doBlockNotify);
        this.wood = wood;
        this.leaves = leaves;
    ...
    public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5)
    {
        ...
        if (par4 >= 1 && par4 + l + 1 <= 256)
        {
            ...
            else
            {
                ...
                boolean isSoil = block2.canSustainPlant(par1World, par3, par4 - 1, par5, ForgeDirection.UP, (BlockSapling)PikminsMoStuff.blockSaplings);
                if (isSoil && par4 < 256 - l - 1)
                {
                        ...
                        for (i2 = par3 - l1; i2 <= par3 + l1; ++i2)
                        {
                            ...
                            for (int k2 = par5 - l1; k2 <= par5 + l1; ++k2)
                            {
                                ...
                                if (Math.abs(j2) != l1 || Math.abs(l2) != l1 || par2Random.nextInt(2) != 0 && i3 != 0)
                                {
                                    ...
                                    {
                                        this.setBlockAndNotifyAdequately(par1World, i2, k1, k2, PikminsMoStuff.blockleaves, this.metaLeaves);
                                    }
                                }
                            }
                        }
                    ...
                    for (k1 = 0; k1 < l; ++k1)
                    {
                        ...
                        {
                            this.setBlockAndNotifyAdequately(par1World, par3, par4 + k1, par5, PikminsMoStuff.blockLogs, this.metaWood);
